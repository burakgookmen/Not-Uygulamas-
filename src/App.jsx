...
function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!bookName) {
      return toast.warning("Lütfen bir not ekleyin!", { autoClose: 1500 });
    }
    ...
    toast.success("Not Eklendi", { autoClose: 1500 });
  };
  ...
  const handleDelete = (deletingId) => {
    ...
    setBooks(filtred);
    toast.error("Not silindi", { autoClose: 1500 });
  };
  ...
  const handleRead = (book) => {
    ...
    toast.warning("Not güncellendi", { autoClose: 1500 });
  };
  ...
  return (
    <div className="body">
      <div className="bg-dark text-light px-5 py-2 fs-5 text-center mb-4 title">
        <span className="worm"></span>Not Ekleyin <span className="worm"></span>
      </div>
      <div className="container ">
        ...
        <div className="d-flex flex-column gap-3 py-5">
          {books.length === 0 && <h4>Not Bulunamadı</h4>}
          {books.map((book) => (
            <BookCard
              key={book.id}
              book={book}
      ...
      {showConfirm && (
        <div className="confirm-modal">
          <div className="modal-inner shadow">
            <h5>Silme konusunda emin misiniz?</h5>
            <button
